...
//______________MIOS XD______________________________________________________________________________________________________________________________________________

void VShaderCopy(float4 vPos : POSITION, float2 vTex : TEXCOORD0, out float4 oPos : POSITION, out float2 oScreenPos : TEXCOORD0)
{
    oPos = vPos;
    oScreenPos = vTex;
    oPos.w = 1;
...
float4 PSGlow(in float2 Tex : TEXCOORD0) : COLOR0
{
...
void PSBlur(float2 screen_pos : TEXCOORD0, out float4 Color : COLOR)
{
    Color = 0;
    for (int i = 0; i < kernel_size; ++i)
        Color += tex2D(RenderTarget, screen_pos + float2(0, (float) (i - kernel_r) / screen_dy)) * Kernel[i];
...
void PSFinal(float2 screen_pos : TEXCOORD0, out float4 Color : COLOR)
{
	float4 base = tex2D(RenderTarget, screen_pos);
    screen_pos.x *= 0.999;
    screen_pos.y *= 1.01;
	float4 glow = tex2D(GlowMap, screen_pos);
...
technique filter4
{
    pass Pass_0
    {
        VertexShader = compile vs_3_0 VShaderCopy();
